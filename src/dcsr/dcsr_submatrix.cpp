...
namespace {
    ...
    void find_rows_range(Controls &controls, uint32_t &rows_begin, uint32_t &rows_end,
        ...
        if (DEBUG_ENABLE) Log() << "GPU rows_begin = " << rows_begin << ", rows_end = " << rows_end;
    }
...
void submatrix(Controls &controls, matrix_dcsr &matrix_out, const matrix_dcsr &matrix_in,
    ...
    if (DEBUG_ENABLE) Log() << "count_subrows_nnz finished in " << t.last_elapsed();
    ...
    if (DEBUG_ENABLE) Log() << "fill_rows finished in " << t.last_elapsed();
    ...
    if (DEBUG_ENABLE) Log() << "rpt_and_rows finished in " << t.last_elapsed();
