...
#define CHECK_CL(expr, status, fast_find_code) try {                                                        \
    expr;                                                                                                   \
    }                                                                                                       \
...
#define CHECK_RAISE(cond, msg, status, fast_find_code) do {                                               \
            if (!(cond)) throw  Exception(msg, status, fast_find_code, __FILE__, __FUNCTION__, __LINE__); \
} while (0);

#define CHECK_CREATE_BUF(expr, fast_find_code) CHECK_CL(expr, CLBOOL_CREATE_BUFFER_ERROR, fast_find_code)
#define CHECK_COPY_BUF(expr, fast_find_code) CHECK_CL(expr, CLBOOL_COPY_BUFFER_ERROR, fast_find_code)

#define CHECK_BUILD(build, fast_find_code) try {                                                                    \
    build;                                                                                                          \
    }                                                                                                               \
    catch (const cl::Error &e)  {                                                                                   \
...
#define CHECK_RUN(run, fast_find_code) CHECK_CL(run, CLBOOL_RUN_KERNEL_FAILURE, fast_find_code)

#define RAISE_ERROR(msg, status, fast_find_code) do { \
