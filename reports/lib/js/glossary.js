...
function lookUpTerminology (word, hasError) {
  ...
  let glossaryDict = {
    ...
    LOOPVIS: {
      "title": "Loops Viewer (Alpha)",
      "text":  "Shows the dynamic scheduled cycle and latency of loop iterations in your design.",
      "secondText": "Loops Viewer data is not avaliable."
    },
    QUARTUS: {
      "title": "",
      "text":  "Compile with the Intel Quartus Prime Compiler to populate this section. Click to see details.",
      "secondText": "Data available after Quartus compile completes."
    },
    QUARTUS_SYCL: {
      "title": "",
      "text":  "Generate an FPGA bitstream to populate this section. Click to see details.",
      "secondText": "Data available after FPGA bitstream generation completes."
    }
  };
  ...
  if (glossaryDict.hasOwnProperty(word)) {
    if (glossaryDict[word].hasOwnProperty("title") && glossaryDict[word].hasOwnProperty("text") &&
      ...
      let titleHtml = "";
      if (glossaryDict[word].title)
        titleHtml = "<b>" + glossaryDict[word].title.replace(PREFIX_PAD, function_prefix) + "</b><br>";
      if (hasError) {
