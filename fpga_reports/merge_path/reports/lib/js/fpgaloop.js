...
var FPGALoop = function(pDiv) {
  ...
  if (!vTmpCard) {
    ...
    fpgaloopTextMsg.innerText = "Click an option in the Loop List pane to see the loop pipeline information.";
  ...
  function addCheckBox() {
    ...
    vTmpInput.addEventListener('change', (event) => {
  ...
  this.draw = function(data) {
    if (!data) return;  // do nothing if data passed is empty
    if (vBodyRef) document.getElementById(vBodyRef).innerHTML = '';
    clearDivContent();
    ...
    vTable.setColumnList('Pipelined', 'Block Scheduled II', 'Block Scheduled fMAX', 'Latency',
