...
var loopIDSaved;
var loopsDataJSONSaved;
var loopsJSONSaved;
var bottleneckJSONSaved;
...
function updateTripCountTree() {
  ...
  renderLoopVis(loopsDataJSONSaved, loopsJSONSaved, bottleneckJSONSaved, loopNodeIDTripCountPairs, loopIDSaved, verificationMode, invocationMap, loopDataNodesPassedInSaved);
}
...
function createTask(node, parentID) {
  ...
  if (debugCreateTask && debugMode) console.log("Adding: " + node.name + ", own ID is: " + node.id + ", parentID is: " + parentID + ", start: " + node.start + ", end: " + node.end);
  ...
  if (node.hasOwnProperty("start") && node.hasOwnProperty("end")) {
    if (isNaN(node.start) || isNaN(node.end)) {
      if (debugCreateTask && debugMode) console.log("Error! Data error: start cycle or latency is not a number: " + node.name);
    ...
    if (debugCreateTask && debugMode) console.log("Warning! Node has no start and end " + node.name);
    return;  // don't add
  }
  ...
  if (node.hasOwnProperty("id")) {
    ...
    if (debugCreateTask && debugMode) console.log("Warning! Node has no ID " + node.name);
    return;
  }
  ...
  if (group === 0) {
    if (debugCreateTask && debugMode) console.log("  is iteration, color: " + nodeColor);
    if (debugCreateTask && debugMode) console.log("  ID: " + node.id);
    ...
    if (debugCreateTask && debugMode) console.log("  curTask ID: " + curTask.getOriginalID());

  } else if (group === 1) {
    if (debugCreateTask && debugMode) console.log("  is not, color: " + nodeColor);
    if (debugCreateTask && debugMode) console.log("  ID: " + node.id);
...
function flattenNode(node, listToUpdate, parentID, sameLevelPredecessorID, sameLevelSuccessor) {
  ...
  if (parseInt(parentID) > 0) newNode["parent_id"] = parentID;
  ...
  if (node.hasOwnProperty("children")) {
    ...
    for (let i=0; i < childrenToVisit.length; i++) {
      ...
      // Switch it back off
      if (latchEncountered) latchEncountered = false;
      let successorNode = {};
      if (i < childrenToVisit.length - 1) successorNode = childrenToVisit[i+1];
...
function flattenJSON(oldJSON, funcNodeID) {
  ...
  var newJSON = [];
  if (debugMode) console.log("CHECKING JSON:");
  if (oldJSON instanceof Array) {
    if (debugMode) console.log("JSON IS AN ARRAY ");
    for (let i=0; i < oldJSON.length; i++) {
      let node = oldJSON[i];
      if (i > 0) flattenNode(node, newJSON, funcNodeID, oldJSON[i-1].id);
      else flattenNode(node, newJSON, funcNodeID, funcNodeID);
    } 
  } else if (oldJSON instanceof Object) {
    if (debugMode) console.log("JSON IS A MAP");
    let oldJSON_values = Object.keys(oldJSON).map(function(key) {
      return oldJSON[key];
    });
    for (let i=0; i < oldJSON_values.length; i++) {
      let node = oldJSON_values[i];
      if (i > 0) flattenNode(node, newJSON, funcNodeID, oldJSON_values[i-1].id);
      else flattenNode(node, newJSON, funcNodeID, funcNodeID);
    } 
  } 
 ...
 * 
 * @param {Object} loopsDataJSON is a list of loops for a given C++ function level
 * @param {Object} loopsJSON is the old format of loop info, used just for speculation currently
 * @param {Object} bottleneckJSON is a list of limiters (memory and data dependencies) and their details
 ...
 */
function renderLoopVis(loopsDataJSON, loopsJSON, bottleneckJSON, loopNodeIDTripCountPairs, loopID, verificationModePassedIn, invocationMapPassedIn, loopDataNodesPassedIn, debugModePassedIn){
  ...
  loopsDataJSONSaved = loopsDataJSON;
  loopsJSONSaved = loopsJSON;
  loopIDSaved = loopID;
  bottleneckJSONSaved = bottleneckJSON;
  invocationMap = invocationMapPassedIn;
  debugMode = debugModePassedIn;
  maxCycle = 0;

  // Clear details pane before rendering
  if (!verificationMode) clearDivContent();

  if (loopID !== undefined && top_node_id === loopID && top_node_id > 0) return;  // do nothing if user clicks the same ID
  top_node_id = loopID;
  
  loopDataNodes = flattenJSON(loopsDataJSON['nodes']);
  loopDataNodesPassedIn = loopDataNodes;
  bottleneckNodes = flattenJSON(bottleneckJSON['bottlenecks']);
  loopLegacyNodes = flattenJSON(loopsJSON['children']);
  taskNodePairsWithTripCounts = [];

  let CID;
  if (verificationModePassedIn) CID = "GanttChartDIV";
  else CID = getViewerConst().gid;
  gantt_chart = new JSGantt.GanttChart(document.getElementById(CID), 1);
  var centerPaneWidth;
  if (verificationModePassedIn) centerPaneWidth = 200;
  else centerPaneWidth = (CID!=null && $('#'+CID).width()> 200? $('#'+CID).width(): 200);  // This is just an error handling, take width=200 if it's smaller than 200px
...
function getMostRecentNodeCopy(node) {
  if (!node.hasOwnProperty("threads")) return {};
...
function createImplicitLimiter(startNode, endNode, startPoint, endPoint, capacity, checkPreviousIteration, parentNode, thresholdDelay) {
  ...
  if (!startNode.hasOwnProperty("implicit_limiter_count")) startNode["implicit_limiter_count"] = 0;
  if (!endNode.hasOwnProperty("implicit_limiter_count")) endNode["implicit_limiter_count"] = 0;
  ...
  if (capacity < 0) capacity = endPoint - startPoint; // Change this, JSON file should contain explicit capacity info
...
function postProcessLimiters(node) {
  if (isComponentOrFunctionNode(node)) return;
  
  // Do the first if statement for both starts/ends
  let allObjects = [];

  if (node.hasOwnProperty("limiter_start_objects")) allObjects = allObjects.concat(node.limiter_start_objects);
...
function doesLimiterExist(limiterStartParentNode, limiterEndParentNode, 
                          limiterStartElement, limiterEndElement, limitedRegionCapacity) {
 ...
 */
function addLimiterToLoopNodes(limiterParentNode,
                       limiterStartParentNode, limiterEndParentNode,
                       limiterStartElement, limiterEndElement, 
                       limitedRegionCapacity, checkPreviousIteration, thresholdDelay) {
  if (checkPreviousIteration === undefined) {
    checkPreviousIteration = false;
  }
  if (thresholdDelay === undefined) {
  ...
  if (isComponentOrFunctionNode(limiterStartParentNode) || isComponentOrFunctionNode(limiterEndParentNode)) return;
  ...
  if (doesLimiterExist(limiterStartParentNode, limiterEndParentNode, limiterStartElement, limiterEndElement, limitedRegionCapacity)) return;
  ...
  if (limiterStartObject.latency < 0) limiterStartObject.latency = 0;
  
  limiterStartParentNode.limiter_start_objects.push(limiterStartObject);
  if (!limiterStartParentNode.limiter_start_objects_map_by_startpoint.hasOwnProperty(limiterStartElement.start)) limiterStartParentNode.limiter_start_objects_map_by_startpoint[limiterStartElement.start] = [];
...
function preProcessNodes() {
  for (let i = 0; i < loopDataNodes.length; i++) {
    ...
    if (isComponentOrFunctionNode(loopNode)) continue;
    ...
    // Loop level/hierarchy
    let parentNode = getNodeFromArray(loopNode.parent_id, loopDataNodes);
    //loopNode["hierarchy"] = parseInt(parentNode.hierarchy) + 1;

    loopNode["threads"] = [];
    loopNode["thread_count"] = 0;
  }

  for (let i = 0; i < loopLegacyNodes.length; i++) {
    let loopLegacyNode = loopLegacyNodes[i];
    
    if(loopLegacyNode.data[2] === "" || loopLegacyNode.data[2] === "n/a") continue;
    
    let partUnrollRegEx = /(.+)X Partially unrolled (.+)/;
    
    let name = loopLegacyNode.name;
    let match = partUnrollRegEx.exec(loopLegacyNode.name);
    if (match != null) {
      name = match[2];
    } 
    
    // Loop level/hierarchy
    let loopNode = getNodeFromArray(name, loopDataNodes);
    loopNode["speculation"] = parseInt(loopLegacyNode.data[2]);
  }
  
  for (let i = 0; i < loopDataNodes.length; i++) {
    let loopNode = loopDataNodes[i];
    let name = loopNode.hasOwnProperty("block_name") ? loopNode.block_name : loopNode.name;
    if (loopNode.hasOwnProperty("ii")) loopNode.ii = parseInt(loopNode.ii);
    if (loopNode.hasOwnProperty("ll")) loopNode.ll = parseInt(loopNode.ll);
    if (loopNode.hasOwnProperty("pl")) loopNode["pipelined"] = loopNode.pl;
    if (loopNode.hasOwnProperty("mi")) loopNode["interleaving"] = parseInt(loopNode.mi);

    if(isComponentOrFunctionNode(loopNode)) continue;
    loopNode["latency"] = parseInt(Math.floor(Number(loopNode.lt)));
    
    // Branch register
    if (shouldRegisterBranch(loopNode)) loopNode.latency += 1;

    console.log("ID: " + loopNode.id + " Latency: " + loopNode.latency);
  }

  // Take bottleneckNodes and update loopDataNodes: loopNode
  for (let i = 0; i < bottleneckNodes.length; i++) {
    // Bottleneck node should have:
    let bottleneckNode = bottleneckNodes[i];
    ...
    if (parseInt(bottleneckNode.src) != parseInt(limiterStartElement.id)) console.error("First limiter node is not the bottleneck source");
    if (parseInt(bottleneckNode.dst) != parseInt(limiterEndElement.id)) console.error("Last limiter node is not the bottleneck destination");
    ...
    if (!limiterEndElement.hasOwnProperty("start")) limiterEndElement["start"] = limiterEndElement.end;
    if (!limiterStartElement.hasOwnProperty("start")) continue; // Dependencies should be fully populated
    if (limiterStartElement.start == "n/a" || limiterEndElement.start == "n/a") continue;
    ...
    let concurrency = 1;
    if (bottleneckNode.hasOwnProperty("concurrency")) concurrency = bottleneckNode.concurrency;
    addLimiterToLoopNodes(limiterParentNode, limiterStartParentNode, limiterEndParentNode,
                       limiterStartElement, limiterEndElement, concurrency, checkPreviousIteration);
  }
  ...
  for (let i = 0; i < loopDataNodes.length; i++) {
    ...
    if (loopNode.hasOwnProperty("children") && loopNode.children.length > 0) {
      ...
      for (let j = 0; j < loopNode.children.length; j++) {
        ...
        if (childLoopNode.type == "loop" && (childLoopNode.pipelined == "Yes" || childLoopNode.pipelined == 1)) {
      ...
      if (loopNode.pipelined == "No" || loopNode.pipelined == 0) {
...
function getLatestElementOfThread(nodeID, threadNum) {
  ...
  if (!nodeCopy.hasOwnProperty("children") || nodeCopy.children.length == 0) {
    ...
    if (!nestedDetails.children_info[lastChildID].hasOwnProperty("invocation_details")) return {};
    let childTripCount = nestedDetails.children_info[lastChildID].invocation_details.speculated_trip_count;
    if (nestedDetails.children_info[lastChildID].iteration_threads.length != childTripCount) return {}; // Child is not done yet!
...
function getPreviousIteration(nodeCopy) {
  if (nodeCopy.iteration_num == 0) return {};
...
function getNestedDetails(nodeID, threadNum) {
  let nodeCopy = getNodeCopyThread(nodeID, threadNum);
  if (!nodeCopy.hasOwnProperty("nested_iter_detail")) console.error("Missing nested iteration details");
...
function getLimiterParentThread(nodeCopy, limiterObj) {
  ...
  if (parseInt(nodeCopy.loop_node_id) == parseInt(limiterParentID)) return nodeCopy.own_thread_num;
...
function isLastInstanceOfLimiterEnd(nodeCopy, limiterObj) {
  ...
  while(true) {
    if (nodeCopyToRecurse.is_last_iteration == false) return -1;
    ...
    if (nodeCopyToRecurse.is_last_iteration && (parentNodeCopy.loop_node_id) == parseInt(limiterParentID)) return nodeCopyToRecurse.parent_thread_num;
...
function initializeNodeCopy(nodeCopy, 
                            node, 
                            iterationIsSpeculative, 
                            scheduleHeader, 
                            parentThreadNum, 
                            iterationNum, 
                            tripCount, 
  ...
  if (!nodeCopy.hasOwnProperty("parent_thread_num")) nodeCopy["parent_thread_num"] = parentThreadNum;
...
function isRealLimiterStartEncounter(nodeCopy, limiterStartObject) {
  if (!limiterStartObject.hasOwnProperty("parent_threads")) limiterStartObject["parent_threads"] = {};
...
function initializeLimiterObjectCopies(node, nodeCopy) {
  if (nodeCopy.hasOwnProperty("limiter_start_objects_copy") && nodeCopy.hasOwnProperty("limiter_end_objects_copy")) return;
  for (let k = 0; k < node.limiter_start_objects.length; k++) {
    ...
    if (limiterStartObject.check_prev_iter == true) {
      if (!nodeCopy.hasOwnProperty("limiter_start_objects_copy")) nodeCopy["limiter_start_objects_copy"] = {};
      if (!nodeCopy.hasOwnProperty("limiter_end_objects_copy")) nodeCopy["limiter_end_objects_copy"] = {};
...
function analyzeLimiterStartObject(node, nodeCopy, limiterStartObject, limiterEndObject, selfNextAvailableStart, predecessorEnd, observedEncounterObj, limitedStartEnd) {
  if (!isRealLimiterStartEncounter(nodeCopy, limiterStartObject)) return;
  ...
  if (limiterStartObject.limited_region_num_threads < limiterStartObject.limited_region_capacity 
    ...
    if (lastStartEncounterIndex < 0 || limiterStartObject.partition_start_encounters.length == 0) lastStartEncounter = -1;
    else lastStartEncounter = limiterStartObject.partition_start_encounters[lastStartEncounterIndex];
    ...
    if (limiterEndObject.partition_start_encounters.length > 0) {
      ...
      if (node.hasOwnProperty("ii") && !isNaN(parseInt(node.ii))) lastStartEncounterPlusII = parseInt(lastStartEncounter) + parseInt(node.ii);
...
function doAllLimiterStartsHaveRequiredInfo(node, nodeCopy) {
  ...
  for (let k = 0; k < node.limiter_start_objects.length; k++) {
    ...
    if (!isRealLimiterStartEncounter(nodeCopy, limiterStartObject)) continue;
...
function initializeNestedIterDetails(node, 
                                     parentNestedDetails,
                                     parentThreadNum,
                                     unspecTripCount,
                                     tripCount,
                                     isSpeculative,
                                     loopTotalLatencyTaskID,
                                     ignorePredecessor) {
...
function scheduleNode(node, 
                      parentThreadNum, 
                      isSpeculative, 
                      parentTaskID, // parentTaskID: parent ID used to call createTask with
                      scheduleHeader, 
                      headerNestedIterDetail, 
  ...
  if (node.hasOwnProperty("children") && node.children.length != 0 && !scheduleHeader && node.type == "loop") {
    ...
    while (incompleteNodes.length > 0) {
      ...
      if (parseInt(childNode.interleaving) > 1) {
        ...
        if (!parentNestedDetails.children_info[node.id].interleaving_details.hasOwnProperty("priority_array")) {
          ...
          for (let i = 0; i < parseInt(childNode.interleaving); i++) {
            if (i < tripCount) parentNestedDetails.children_info[node.id].interleaving_details.priority_array.push(i);
          }
        } 
        ...
        if (!childNode.hasOwnProperty("check_mode")) childNode["check_mode"] = true;
        if (!childNode.hasOwnProperty("first_checked")) childNode["first_checked"] = parentThreadNum;
        ...
        if(priorityArray.length > 0) {
          ...
          if (childNode.check_mode) {
            ...
            for (let i = 0; i < priorityArray.length; i++) {
              ...
              if (parseInt(currObservedEncounter) < 0) continue;
          ...
          // 2) If we have already observed, decide if this is our time to set
            if (earliestStartEncounter < 0) {
              ...
              if (priorityArray.length > 0) scheduleComplete = false;    
            }
          }
        }
      ...
      if (!scheduleComplete && isComponentOrFunctionNode(parentNode)) incompleteNodes.push(childNode.id);
      if (!scheduleComplete) allChildrenComplete = false;
    }
  ...
  for (let i = 0; i < tripCount; i++) {
    ...
    if ((scheduleHeader == false && parseInt(parentNestedDetails.children_info[node.id].iteration_threads.length) >= parseInt(i)+1)) {
      ...
      if (iterThreadObj.done == true) continue;
      ...
      if (scheduleHeader) initializeNodeCopy(nodeCopy, node, iterationIsSpeculative, scheduleHeader, parentThreadNum, headerIterNum, parentNestedDetails.children_info[node.id].invocation_details.speculated_trip_count, headerNestedIterDetail);
      else initializeNodeCopy(nodeCopy, node, iterationIsSpeculative, scheduleHeader, parentThreadNum, i, tripCount, headerNestedIterDetail);
      iterThreadObj = parentNestedDetails.children_info[node.id].iteration_threads[i];
    }

    if (nodeCopy.hasOwnProperty("start") && nodeCopy.hasOwnProperty("end")) continue;
    ...
    if (!ignorePredecessor && Object.entries(predecessorNode).length > 0) {
      ...
      if (Object.entries(mostRecentPredecessorNodeCopy).length > 0 ) {
        if(!mostRecentPredecessorNodeCopy.hasOwnProperty("end")) {
          if (debugMode) console.log("Predecessor isn't done yet");
    ...
    if (node.hasOwnProperty("limiter_start_objects")) {
      ...
      if (!doAllLimiterStartsHaveRequiredInfo(node, nodeCopy)) {
        if (debugMode) console.log("Limiter starts do not all have required info");
        return false;
      }
      ...
      for (let k = 0; k < node.limiter_start_objects.length; k++) {
        ...
        if (limiterStartObject.check_prev_iter == true) {
          ...
          if (Object.entries(nodeCopyPrevIter).length == 0) continue;
          else limiterEndObject = nodeCopyPrevIter.limiter_end_objects_copy[limiterEndObject.id];
        }
    ...
    if (Object.entries(observedEncounterObj).length > 0) {
      ...
      if (debugMode) console.log("Observing start: " + observedEncounterObj.encounter);
      return false;
    }
        ...
        if (limiterEndObject.check_prev_iter) {
          ...
          if (limiterEndObject.last_start_point == 0 && (!limiterEndNode.hasOwnProperty("limiter_start_objects_map_by_startpoint") || !limiterEndNode.limiter_start_objects_map_by_startpoint.hasOwnProperty(0))) {
            ...
            for (let i = 0; i < startObjectsArrLength; i++) {
              ...
              if (lastStartEncounterIndex >= 0) lastStallPointStart = Math.max(parseInt(lastStallPointStart), parseInt(startObject.partition_start_encounters[lastStartEncounterIndex])); // Last/Most recent one (do we even need the ones before that)
            }
          }
    ...
    if (node.hasOwnProperty("ii") && !isNaN(parseInt(node.ii))) node.next_available_start = nodeCopy.start + node.ii;
    else node.next_available_start = nodeCopy.start + node.latency; // If there's no II details, assume II = latency
    
    nodeCopy.next_available_start = node.next_available_start;
    
    if (node.type == "loop" && !scheduleHeader) nodeCopy.id = getAvailableTaskID();
    else if (scheduleHeader == true) nodeCopy.id = getAvailableTaskID();
    else nodeCopy.id = getAvailableTaskID();

    if (node.type == "loop" && scheduleHeader) nodeCopy["name"] = "Header (" + nodeCopy.name + ")";
    if (node.type == "loop_latch") nodeCopy["name"] = "Latch (" + nodeCopy.name + ")";
    ...
    if (node.type == "loop" && !scheduleHeader) {
      if (debugMode) console.log("\t\tLOOP, NOT SCHEDULING A HEADER");
      createTask(nodeCopy, loopTotalLatencyTaskID);
    }
    else {
      if (debugMode) console.log("\t\tSCHEDULING A HEADER OR OTHER");
    ...
    if (parseInt(node.interleaving) > 1 && i < (parseInt(tripCount) - 1)) return false; 
  }
...
function initializeTripCountTreeNode(ownLoopNode, tripCount, parentTCNode) {
  ...
  if (Object.entries(parentTCNode).length > 0) {
    if (!parentTCNode.children.hasOwnProperty(ownLoopNode.id)) parentTCNode.children[ownLoopNode.id] = [];
    parentTCNode.children[ownLoopNode.id].push(tripCountNode);
  }
  
  if (!tripCountNodeMap.hasOwnProperty(ownLoopNode.id)) tripCountNodeMap[ownLoopNode.id] = [];
...
function fillupTripCountTreeNode(tripCountNode) {
  ...
  if (Object.entries(tripCountNode.children) == 0) return;
...
function updateLoopNodeTripCounts(tripCountNode) {
  ...
  if (!loopNode.hasOwnProperty("trip_count_nodes")) loopNode["trip_count_nodes"] = [];
 ...
 */
function initializeTripCounts(constValue, loopNodeIDTripCountPairs) {
  tripCountNodeMap = {};
  ...
  if (loopNodesToAnalyze.length == 0) loopNodesToAnalyze = loopDataNodes;
  ...
  for (let i = 0; i < loopNodesToAnalyze.length; i++) {
    ...
    if (loopNodeObj.hasOwnProperty("loop_node") && loopNodeObj.hasOwnProperty("trip_count")) {
      ...
      if (!isNaN(loopNodeTripCount) && loopNodeTripCount > 0) {
        ...
        if (loopNodeTripCount <= 100) {
          loopNode["threshold_delay"] = 0;
        }
      }
    }
...
function populateIterations(loopNodeIDTripCountPairs, selectedLoopID) {
  ...
  // For every provided trip count (initialize to 1 for each)
  initializeTripCounts(1, loopNodeIDTripCountPairs);
  preProcessNodes();
  ...
  for (let i = 0; i < loopDataNodes.length; i++) {
    ...
    if (isComponentOrFunctionNode(loopNode)) {
      ...
      if (selectedLoopID < 0) createTask(loopNode, 0);     
      loopNode["threads"] = [];
      loopNode.threads.push(loopNode);
      if (debugMode) console.log("Component Node: " + loopNode.id);
      continue;
    }
    ...
    if (loopNode.ll == 1) { // If parent is component?
      ...
      if (debugMode) console.log("Parent ID:" + loopNode.parent_id + "  Parent type: " + componentNode.type);
